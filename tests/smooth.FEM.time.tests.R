...
output_CPP_it<-smooth.FEM.time(locations = locations, time_mesh = TimePoints, 
                            observations=observations, 
                            covariates = cov1,
                            FEMbasis=FEMbasis, lambdaS=lambdaS, lambdaT=lambdaT,
                            FLAG_PARABOLIC = FALSE, FLAG_ITERATIVE = TRUE, FLAG_FINITEDIFFERENCES = TRUE)
plot(output_CPP_it$fit.FEM)
image(output_CPP_sol$fit.FEM, t = pi)
image(output_CPP_sp$fit.FEM, t = pi)
image(output_CPP_mon$fit.FEM, t = pi)
image(output_CPP_it$fit.FEM, t = pi)
...
output_CPP_it<-smooth.FEM.time(time_mesh = TimePoints, observations=observations, 
                            FEMbasis=FEMbasis, lambdaS=lambdaS, lambdaT=lambdaT,
                            PDE_parameters=PDE_parameters, FLAG_PARABOLIC=FALSE,
                            FLAG_ITERATIVE = TRUE,
                            FLAG_FINITEDIFFERENCES = TRUE)
image(FEM.time(sol_exact, time_mesh = TimePoints, FEMbasis = FEMbasis, FLAG_PARABOLIC = TRUE), t=1.4)
image(output_CPP_sp$fit.FEM, t = 0)
image(output_CPP_it$fit.FEM, t = 2)
image(output_CPP_it$fit.FEM, t = 2)

image(FEM.time(sol_exact, time_mesh = TimePoints, FEMbasis = FEMbasis, FLAG_PARABOLIC = TRUE), t=0)
...
#plot(mesh)
FEMbasis = create.FEM.basis(mesh)
time_mesh=seq(0,4.0,length.out = 11)
...
output_CPP<-smooth.FEM.time(observations=observations,
                            ...
                            FLAG_FINITEDIFFERENCES = TRUE,
                            DOF.evaluation = NULL)
for (tempo in time_mesh)
  plot(output_CPP$fit.FEM.time, t = tempo)
...
output_CPP<-smooth.FEM.time(observations=observations,
                            ...
                            FLAG_PARABOLIC = TRUE,
                            lambda.selection.criterion='grid', DOF.evaluation='exact', lambda.selection.lossfunction='GCV')
plot(output_CPP$fit.FEM.time, t = 0.5)
#### iterative method
output_CPP_iter<-smooth.FEM.time(observations=observations,
                            incidence_matrix = incidence_matrix, areal.data.avg = TRUE,
                            FEMbasis = FEMbasis, time_mesh = time_mesh,
                            ...
                            BC = BC,
                            PDE_parameters = PDE_parameters,
                            FLAG_PARABOLIC = FALSE, FLAG_ITERATIVE = FALSE, FLAG_FINITEDIFFERENCES = TRUE,
                            lambda.selection.criterion='grid', DOF.evaluation='exact', lambda.selection.lossfunction='GCV')
plot(output_CPP_iter$fit.FEM.time, t = 0.5)
...
output_CPP_iter<-smooth.FEM.time(observations=observations,
                            ...
                            BC = BC,
                            PDE_parameters = PDE_parameters,
                            FLAG_PARABOLIC = TRUE, FLAG_ITERATIVE = FALSE, FLAG_FINITEDIFFERENCES = FALSE)
...
output_CPP_iter<-smooth.FEM.time(observations=observations,
                            ...
                            BC = BC,
                            PDE_parameters = PDE_parameters,
                            FLAG_PARABOLIC = TRUE, FLAG_ITERATIVE = TRUE, FLAG_FINITEDIFFERENCES = TRUE)
plot(output_CPP_iter$fit.FEM.time, t = 1)
...
#### iterative method
output_CPP_fd<-smooth.FEM.time(observations=observations,
                            covariates = cov1,
                            incidence_matrix = incidence_matrix, areal.data.avg = TRUE,
                            FEMbasis = FEMbasis, time_mesh = time_mesh,
                            lambdaS = lambdaS, lambdaT = lambdaT,
...
output_CPP_fd$inference$beta$p_values
output_CPP_fd$inference$beta$CI
...
plot(FEM.time(coeff=array(func_evaluation,dim=c(length(func_evaluation),1,1)), FEMbasis = FEMbasis,time_mesh=TimeNodes,FLAG_PARABOLIC=T),t = 0)
...
# Plot exact solution
plot(FEM.time(coeff=array(W%*%beta_exact + func_evaluation, dim = c(nnodes*length(TimeNodes),1,1)),FEMbasis=FEMbasis,time_mesh = TimeNodes,FLAG_PARABOLIC = T),t = 2)
...
solSepIt = smooth.FEM.time(observations=data,
                         FEMbasis = FEMbasis, time_mesh = TimeNodes, time_locations = TimeNodes,
                         lambdaS = lambdaS, lambdaT = lambdaT, 
                         FLAG_PARABOLIC = FALSE, FLAG_ITERATIVE = TRUE, FLAG_FINITEDIFFERENCES = TRUE)
plot(solSepSp$fit.FEM.time, t = 0)

solSepIt = smooth.FEM.time(observations=datacov, covariates = W,
                            FEMbasis = FEMbasis, time_mesh = TimeNodes,
                            lambdaS = 10^-2.5, lambdaT = 10^-3,
                            FLAG_PARABOLIC = FALSE, FLAG_ITERATIVE = TRUE, FLAG_FINITEDIFFERENCES = TRUE)
plot(solSepCovFd$fit.FEM.time, t = 0)


plot(FEM.time(coeff=array(func_evaluation,dim=c(length(func_evaluation),1,1)), FEMbasis = FEMbasis,time_mesh=TimeNodes,FLAG_PARABOLIC=T),t = 4)
plot(solSepSp$fit.FEM.time, t = 0)
plot(solSepMon$fit.FEM.time, t = 4)
plot(solSepIt$fit.FEM.time, t = 4)
...
solParIter = smooth.FEM.time(observations=data,
                         ...
                         FLAG_PARABOLIC = TRUE, FLAG_ITERATIVE = TRUE, FLAG_FINITEDIFFERENCES = FALSE)
...
plot(FEM.time(coeff=array(func_evaluation,dim=c(length(func_evaluation),1,1)),time_mesh = TimeNodesRMSE,FEMbasis,FLAG_PARABOLIC = T),t = 1)
...
solSep = smooth.FEM.time(observations = data, incidence_matrix = incidence_matrix, time_locations=TimeNodes,
                         ...
                         lambdaS = lambdaS, lambdaT = lambdaT, FLAG_PARABOLIC = FALSE,
                         FLAG_ITERATIVE = TRUE, FLAG_FINITEDIFFERENCES = TRUE)
plot(solSep$fit.FEM.time, t = 1)

solSepCov = smooth.FEM.time(observations = datacov,time_mesh = TimeNodes, covariates = W_areal,incidence_matrix = incidence_matrix,
                         FEMbasis = FEMbasis, lambdaS = lambdaS, lambdaT = lambdaT, FLAG_PARABOLIC = FALSE,
                         FLAG_ITERATIVE = FALSE, FLAG_FINITEDIFFERENCES = FALSE)
plot(solSepCov$fit.FEM.time, t = 1)
...
### GCV Exact
solParIter = smooth.FEM.time(observations=data,
                             FEMbasis = FEMbasis, time_mesh = TimeNodes, incidence_matrix = incidence_matrix, time_locations =TimeNodes, lambdaS = lambdaS_par, lambdaT = lambdaT_par, FLAG_PARABOLIC = FALSE, FLAG_ITERATIVE =TRUE,FLAG_FINITEDIFFERENCES = TRUE,
                             lambda.selection.criterion='grid',DOF.evaluation='exact',lambda.selection.lossfunction='GCV')

### GCV Stochastic 
solParIter = smooth.FEM.time(observations=data,
                             FEMbasis = FEMbasis, time_mesh = TimeNodes, time_locations =TimeNodes, incidence_matrix = incidence_matrix,
                             lambdaS = lambdaS, lambdaT = lambdaT,FLAG_PARABOLIC = FALSE, FLAG_ITERATIVE =FALSE, FLAG_FINITEDIFFERENCES = TRUE,
                             lambda.selection.criterion='grid',DOF.evaluation='stochastic',lambda.selection.lossfunction='GCV')
plot(solParIter$fit.FEM.time, t = 1)
...
solSepSp = smooth.FEM.time(observations = datacov,time_mesh = TimeLocations,
                         FEMbasis = FEMbasis, lambdaS = lambdaS, lambdaT = lambdaT, FLAG_PARABOLIC = FALSE,
                         FLAG_ITERATIVE = FALSE, FLAG_FINITEDIFFERENCES = FALSE)

sol_eval=eval.FEM.time(solSep$fit.FEM.time,locations = loc[1:nloc,2:4], time.instants = timeloc, lambdaS = solSep$optimization$lambda_position[1], lambdaT = solSep$optimization$lambda_position[1])
sol_exact = func_evaluation2
RMSE<-function(f,g) sqrt(mean((f-g)^2))
RMSE(sol_eval,sol_exact)

plot(FEM(func_evaluation[1:nnodes],FEMbasis), t=0.2)
plot(solSepSp$fit.FEM.time, t = 0.2)
plot(solSepMon$fit.FEM.time, t = 0.2)
plot(solSepIt$fit.FEM.time, t = 0.2)
...
solSepCov = smooth.FEM.time(observations = datacov,time_mesh = TimeLocations, covariates = W,
                            ...
                            FLAG_ITERATIVE = TRUE, FLAG_FINITEDIFFERENCES = FALSE)
...
solSep = smooth.FEM.time(observations=data,
                         FEMbasis = FEMbasis, time_mesh = TimeNodes, time_locations = TimeNodes,
                         lambdaS = lambdaS, lambdaT = lambdaT, FLAG_PARABOLIC = TRUE,
                         FLAG_ITERATIVE = TRUE, FLAG_FINITEDIFFERENCES = FALSE)
